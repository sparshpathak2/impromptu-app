...
import { Modal, Grid, TextInput, Flex, Menu, Button, UnstyledButton, Badge, Text, Group, ActionIcon, Tooltip, rem, Anchor } from '@mantine/core';
...
import classes from './LeftNavDashboard.module.css';
...
function CreateCollectionModal({ handleCreateCollection, closeCreateCollectionModal, modalCreateCollectionOpened }) {
    ...
    return (
        <>
            <Modal opened={modalCreateCollectionOpened} onClose={closeCreateCollectionModal} yOffset="15vh" size="36%" title="Create a Collection">
...
function DeleteCollectionModal({ modalDeleteCollectionOpened, closeDeleteCollectionModal, handleDeleteCollection }) {
    ...
    return (
        <>
            <Modal opened={modalDeleteCollectionOpened} onClose={closeDeleteCollectionModal} yOffset="15vh" size="28%" withCloseButton={false}>
...
function EditCollectionModal({ handleUpdateCollection, modalEditCollectionOpened, closeEditCollectionModal, foldersState, editingFolderId, editModalOpen }) {
    ...
    // React.useEffect(() => {
    //     editingFolder = workspace[workspaceLastIndex]?.folders.find(folder => folder._id === editingFolderId.itemId)
    ...
    //     if (editingFolder) {
    //         // setFolderName(editingFolder.folder);
    //         setInputValue(editingFolder.folder);
    //     }
    // }, [inputValue, editModalOpen]);
    ...
    return (
        <>
            <Modal opened={modalEditCollectionOpened} onClose={closeEditCollectionModal} yOffset="15vh" size="36%" title="Edit Collection">
                <Flex direction='column' align='center' gap='16px'>
                    <Flex align='center' gap={8}>
                        {/* <ActionIcon variant="default" radius="xl" size="xl" color="red"> */}
                        {/* <IconExclamationCircle size={48} stroke={1} color='red' /> */}
...
export function LeftNavDashboard({ workspace, folderId, foldersState, setFoldersState }) {
        ...
        <nav className={classes.navbar}>
            ...
            {/* <div className={classes.section}>
                <div className={classes.mainLinks}>
                    <Menu className={classes.mainLink} width="target" offset={14} shadow="xl">
                        <Menu.Target>
                            <UnstyledButton>
                                <Flex w='100%' justify='space-between' align='center'>
                                    <Flex gap={8} w='100%'>
                                        <Badge radius="sm" variant="light">S</Badge>
                                        <Text size="sm">{workspace[0]?.name}'s Workspace</Text>
                                    </Flex>
                        ...
                        <Menu.Dropdown w='auto'>
                            <Menu.Item leftSection={<IconBulb style={{ width: rem(14), height: rem(14), color: "#495057" }} />}>
                                <Text size='sm' c="#495057">Settings</Text>
                            </Menu.Item>
                            <Menu.Item leftSection={<IconBulb style={{ width: rem(14), height: rem(14), color: "#495057" }} />}>
                                <Text size='sm' c="#495057">Settings</Text>
                            </Menu.Item>
            ...
            <div className={classes.section}>
                <Group className={classes.collectionsHeader} justify="space-between">
                    ...
                    {/* <Modal opened={opened} onClose={close} yOffset="15vh" size="36%" title="Create a Collection">
                        <Flex direction='column' align='center' gap='16px'>
                            <Flex align='center' gap={8}>
                                <ActionIcon variant="default" radius="xl" size="xl" color="red">
                                <IconExclamationCircle size={48} stroke={1} color='red' />
                ...
                {/* {workspace[0]?.folders?.map((folder) => (
                <div className={classes.collections}>{folder}</div>    
                ))} */}
